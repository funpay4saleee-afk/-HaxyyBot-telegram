...
DOWNLOAD_PATH = "/tmp/bot_media"
CACHE_PATH = "/tmp/bot_cache"
...
async def get_video_title(url):
    try:
        result = subprocess.run(["yt-dlp", "--get-title", "--no-playlist", url], capture_output=True, text=True, timeout=5)
    ...
    if not downloaded:
        ...
        try:
            subprocess.run([
                ...
                "-o", final_mp3, "--no-playlist", "--no-check-certificates", "-N", "8", url
    ...
    if data == "video":
        ...
        try:
            subprocess.run([
                ...
                "-o", output, "--no-playlist", "--no-check-certificates", "-N", "8", url
            ], timeout=60, capture_output=True)
            anim_task.cancel()
            try: await anim_task
            except: pass
